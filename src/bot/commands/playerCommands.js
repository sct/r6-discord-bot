...
export const commandPlayerStats = async function commandPlayerStats(
  message,
  playerName
) {
  try {
    ...
    if (player.kills) {
      ...
      if (player.name.toLowerCase() === "keep_one") {
        embed.addField(
          "Time Spent Outside Droning",
          `${470 + addedHours} hours`
        );
        embed.addField(
          "Time Spent Not Attacking",
          `${550 + addedHours * 1.5} hours`
        );
    ...
    return message.channel.send(
      "Unable to load statistics for requested player"
    );
  } catch (error) {
